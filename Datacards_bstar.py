...
CARDDIR     = "datacards/"+BTAGGING+"/bstar/"
YEAR        = options.year
ISMC        = options.isMC
ABSOLUTEPATH= "."
PLOTDIR     = "plots/datacards/bstar/"
...
def generate_datacard(year, category, masspoint, btagging, outname):
    ...
    card += "kmax *\n"
    card += "-----------------------------------------------------------------------------------\n"
    card += "shapes         {sname}  *    {path}/../workspace/{btagging}/M_bstar_{year}_{category}.root     bstar_{year}:{sname}\n".format(sname=signalName, year=year, category=category, btagging=btagging, path=ABSOLUTEPATH)
